...
def build_prompt(
    ...
    ※ 선호 활동에 포함된 카테고리의 장소는 무조건 is_valid=true로 처리하세요.
      예) 액티비티 선택 → 사격장, 클라이밍, 번지점프, 체험형 장소 등 무조건 true
...
{snippets}""")
    ...
    ※ 무조건 true로 처리하는 경우:
      - 음식점, 카페 관련 장소 (카테고리에 음식점/카페 포함)
      - 유저가 선택한 활동({", ".join(activities_kr)})과 관련된 장소
        예) 액티비티 선택 → 사격장, 클라이밍, 체험형 시설 무조건 true
      - 조금이라도 애매한 경우

    ※ false로 처리하는 경우 (명확할 때만):
      - 여행/관광과 전혀 무관한 시설
