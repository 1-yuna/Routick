# ─────────────────────────────────────────────────────────────────────
# place_keywords
# ─────────────────────────────────────────────────────────────────────
# 키워드 관련 상수 매핑 모음
#
# - KEYWORD_EXPANSIONS          : 키워드 → 동의어/유사어 확장 (검색 확장)
# - NAME_SEARCH_KEYWORDS        : activities별 name 검색 전용 키워드
# - PRIORITY_KEYWORDS           : 활동/동행자별 우선순위 키워드
# - ACTIVITY_EXCLUDE_KEYWORDS   : activities 선택 여부 기반 제외 키워드
# - EXCLUDE_KEYWORDS            : 여행과 무관한 제외 키워드 (category)
# - EXCLUDE_KEYWORDS_NAME       : 여행과 무관한 제외 키워드 (name)
# ─────────────────────────────────────────────────────────────────────

# ─── 키워드 → 동의어/유사어 확장 (검색 확장) ───
KEYWORD_EXPANSIONS = {
    # activities 8개 확장
    "전시/예술":    ["박물관", "전시관", "미술관", "문화원", "전시회", "박람회", "화랑"],
    "공연/문화":    ["시장", "영화관", "서점", "전시관", "문화원", "공연장", "연극극장", "화랑"],
    "액티비티":     ["공원시설물", "사진관", "아쿠아리움", "행글라이딩", "패러글라이딩",
                 "서바이벌게임", "테마파크", "해수욕장", "실내동물원"],
    "실내오락":     ["사진관", "만화카페", "보드카페", "방탈출카페", "오락실", "멀티방"],
    "쇼핑":         ["사진관", "백화점", "의류판매"],
    "공방/소품":    ["사진관", "도자기", "수예", "자수", "가죽공예", "목공예", "취미용품점", "기념품판매", "인테리어장식판매"],
    "자연/관광":    ["공원", "공원시설물", "관광", "명소"],
    "술/바":        [],
}

# ─── activities별 name 검색 전용 키워드 (검색 확장) ───
NAME_SEARCH_KEYWORDS = {
    "액티비티":  ["짚라인", "짚트랙", "짚와이어", "아라나비"],
    "공방/소품": ["공방", "원데이", "팝업"],
    "실내오락":  ["플스", "공방", "원데이", "팝업", "파티룸"],
}

# ─── 여행과 무관한 제외 키워드 (name에서만 매칭) ───
EXCLUDE_KEYWORDS_NAME = [
    "홀덤펍", "체육센터", "헬스클럽", "스튜디오",  "센터",
    "피시방", "PC방", "안내소", "연구소", "카지노", "광장", "골목", "체육", "축구",
]

# ─── activities 선택 여부 기반 제외 키워드 (category에서만 매칭) ───
ACTIVITY_EXCLUDE_KEYWORDS = {
    "전시/예술":  {"exclude_if_not_selected": ["박물관", "전시관", "미술관", "문화원", "전시회", "박람회", "화랑"]},
    "공연/문화":  {"exclude_if_not_selected": ["시장", "영화관", "서점", "전시관", "문화원", "공연장", "연극극장", "화랑"]},
    "액티비티":   {"exclude_if_not_selected": ["공원시설물", "사진관", "아쿠아리움", "행글라이딩", "패러글라이딩",
                                           "서바이벌게임", "테마파크", "해수욕장", "실내동물원"]},
    "실내오락":   {"exclude_if_not_selected": ["사진관", "만화카페", "보드카페", "방탈출카페", "오락실", "게임방", "PC방", "멀티방"]},
    "쇼핑":       {"exclude_if_not_selected": ["사진관", "백화점", "의류판매"]},
    "공방/소품":  {"exclude_if_not_selected": ["사진관", "도자기", "수예", "자수", "가죽공예", "목공예", "취미용품점", "기념품판매", "인테리어장식판매"]},
    "자연/관광":  {"exclude_if_not_selected": ["공원", "공원시설물", "관광", "명소"]},
    "술/바":      {"exclude_if_not_selected": ["술집"]},
    "자녀":       {"exclude_if_not_selected": ["놀이교육", "키즈카페"]},
}

# ─── 활동/동행자별 우선순위 키워드 ───
PRIORITY_KEYWORDS = {
    "술/바":        {"name": [], "category": ["술집", "바", "호프", "이자카야", "칵테일"]},
    "연인":         {"name": ["애견카페", "고양이카페"], "category": ["애견카페", "고양이카페"]},
    "반려동물과":   {"name": ["애견카페", "펫파크", "반려동물"], "category": ["애견카페", "공원", "해변", "산책로", "반려동물"]},
    "자녀와":       {"name": ["키즈카페", "놀이교육"], "category": ["키즈카페", "놀이교육"]},
    "부모님과":     {"name": ["전통찻집"], "category": ["전통찻집"]},
}


# ─── 여행과 무관한 제외 키워드 (category에서만 매칭) ───
EXCLUDE_KEYWORDS = [
    # 생활편의
    "다방", "화장실", "친목", "편의점", "슈퍼마켓", "상가", "아케이드",
    "꽃집", "꽃배달", "세탁소", "문구", "사무용품", "할인매장", "복권",
    "식품판매", "통신판매", "가구판매", "전자제품", "보석", "귀금속", "패션잡화점",

    # 의료/건강
    "병원", "의료", "건강",

    # 숙박/부동산
    "숙박", "분양", "부동산",

    # 교육
    "학원", "온라인교육", "학교부속시설", "직업전문교육", "학습시설", "교육단체",

    # 금융/보험
    "금융", "보험",

    # 교통/수송
    "교통", "수송",

    # 스포츠/운동시설
    "테니스", "당구", "골프", "볼링", "무예", "요가", "필라테스", "사격", "피트니스", "피티니스",
    "스포츠용품", "헬스클럽", "운동장", "스포츠센터", "스포츠시설", "체육관", "수영",

    # 공공/행정
    "사회", "공공기관", "센터",

    # 기타
    "노래방", "미용", "유흥시설", "장례", "샘터", "약수터", "다이빙대",
    "영상제작", "종교", "무용", "댄스", "음악", "출판사", "액자", "화방",
    "제조업", "쓰레기", "담배", "전문대행", "인터넷", "관리",
    "건설", "건축", "여행사", "정보통신", "마케팅", "광고디자인", "거리",
]